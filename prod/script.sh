...
NAMESPACE="ambiente-atales-prod"

echo -e "${GREEN}\n🚀 INICIANDO CLÚSTER ATALES PROD (Minikube + SealedSecrets + ArgoCD)${NC}"
...
# 4. Restaurar clave privada para Prod
if [ -f sealed-secrets-private-key-prod.yaml ]; then
  echo -e "${YELLOW}🛡️ Restaurando clave privada de Sealed Secrets Prod...${NC}"
  kubectl apply -f sealed-secrets-private-key-prod.yaml -n kube-system
  echo -e "${GREEN}✅ Clave privada Prod restaurada${NC}"
else
  echo -e "${RED}❌ No se encontró sealed-secrets-private-key-prod.yaml en prod${NC}"
  exit 1
fi
...
if [ ! -f "$SEALED_SECRET_FILE" ]; then
  echo -e "${RED}❌ No se encontró $SEALED_SECRET_FILE en prod${NC}"
  exit 1
fi
...
# 9. Aplicar aplicación Argo CD para Prod
echo -e "${BLUE}\n🚀 Aplicando aplicación Argo CD prod...${NC}"
kubectl apply -f argo-apps/aplicationAtalesProd.yaml -n argocd
...
echo -e "${GREEN}✅ CLÚSTER PROD LISTO\n"
