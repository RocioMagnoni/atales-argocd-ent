...
NAMESPACE="ambiente-atales-dev"

echo -e "${GREEN}\n🚀 INICIANDO CLÚSTER ATALES DEV (Minikube + SealedSecrets + ArgoCD)${NC}"
...
# 4. Restaurar clave privada para Dev
if [ -f sealed-secrets-private-key.yaml ]; then
  echo -e "${YELLOW}🛡️ Restaurando clave privada de Sealed Secrets Dev...${NC}"
  kubectl apply -f sealed-secrets-private-key.yaml -n kube-system
  echo -e "${GREEN}✅ Clave privada Dev restaurada${NC}"
else
  echo -e "${RED}❌ No se encontró sealed-secrets-private-key.yaml en dev${NC}"
  exit 1
fi
...
if [ ! -f "$SEALED_SECRET_FILE" ]; then
  echo -e "${RED}❌ No se encontró $SEALED_SECRET_FILE en dev${NC}"
  exit 1
fi
...
# 9. Aplicar aplicación Argo CD para Dev
echo -e "${BLUE}\n🚀 Aplicando aplicación Argo CD dev...${NC}"
kubectl apply -f argo-apps/aplicationAtalesDev.yaml -n argocd
...
echo -e "${GREEN}✅ CLÚSTER DEV LISTO\n"
